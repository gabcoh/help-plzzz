...
def get_index(request, class_pk):
    ...
    if klass.can_view_private(request.user):
        ...
        request_list = requests.filter(public=True).order_by('-pub_datetime')[:]
...
def request_help(request, class_pk):
    help_request = HelpRequest(topic=request.POST['topic'],
            creator=request.user, public=('public' in request.POST),
...
def join_class(request):
    ...
    if not all(map(lambda field: field in request.POST, fields)):
        ...
        klass.save()
        return render(request, "help_plz/account.html", { 'recent_requests':recent_requests(request.user)})
@require_http_methods(["GET","POST"])
@login_required()
